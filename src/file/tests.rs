...
async fn do_get_files_from_albumtracks(db: SqlitePool) {
    ...
    let song_1 = Song::lookup_by_id(&db, song_id_1).await.unwrap();
    ...
    let albumtracks = AlbumTrack::load_from_album(&db, &album).await.unwrap();
    let files = File::load_from_albumtracks(&db, &albumtracks).await.unwrap();
